...
@section('title', 'Edit Membership - Gym Management')

@section('content')
<div class="row mb-4">
    <div class="col-12">
        <h2 class="text-secondary">Edit Membership</h2>
        <p class="text-muted">Update membership information</p>
    </div>
</div>
        ...
        <form action="{{ route('memberships.update', $membership) }}" method="POST">
                ...
                <select class="form-select @error('member_id') is-invalid @enderror" 
                    ...
                    @foreach($members as $member)
                        <option value="{{ $member->id }}" 
                            {{ old('member_id', $membership->member_id) == $member->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('plan_type') is-invalid @enderror" 
                    ...
                    <option value="Quarterly" {{ old('plan_type', $membership->plan_type) == 'Quarterly' ? 'selected' : '' }}>Quarterly</option>
                    <option value="Semi-Annual" {{ old('plan_type', $membership->plan_type) == 'Semi-Annual' ? 'selected' : '' }}>Semi-Annual</option>
                    <option value="Annual" {{ old('plan_type', $membership->plan_type) == 'Annual' ? 'selected' : '' }}>Annual</option>
                </select>
                @error('plan_type')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" 
                       id="amount" name="amount" value="{{ old('amount', $membership->amount) }}" required>
                @error('amount')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                           ...
                           value="{{ old('start_date', $membership->start_date->format('Y-m-d')) }}" required>
                    @error('start_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="d-flex gap-2">
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-save"></i> Update Membership
                </button>
